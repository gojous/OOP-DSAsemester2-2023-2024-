...
public final class MoveOrdering {
    ...
    private MoveOrdering() {
        this.evaluator = new StandardBoardEvaluator.get();
    }  
    ...
    private int calculateAttackBonus(final Player player,final Move move){
        final int  attackBonus=move.isAttack() ? 1:0;
        return attackBonus*(player.getOpponent().isWhite()?1:-1);
    }
    private static Collection<Move> calculateSimpleMoveOrder(final Collection<Move> moves){
        ...
        Collection.sort(sortedMoves,(m1,m2)->Booleans.compare(m2.isAttack(),m1.isAttack()));
        return sortedMoves;
    }

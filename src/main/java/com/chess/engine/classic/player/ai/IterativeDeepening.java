...
public class IterativeDeepening extends Observer implements MoveStrategy{
    ...
    public Move execute(final Board board){
        ...
        for(final Move move: board.currentPlayer().getLegalMoves()){
            builder.addMoveOrdering(move,0);
        }
        ...
        while(currentDepth<= this.searchDepth){
            ...
            for(final MoveScoreRecord record:records){
                ...
                if(moveTransition.getMoveStatus().isDone()){
                    ...
                    builder.addMoveOrdering(move,currentValue);
        ...
        this.executionTime, (1000*((double)this.boardsEvaluated/this.executionTime)),
    ...
    final int lowest){
        ...
        for(final Move move: this.moveSorter.sort(board.currentPlayer().getLegalMoves())){
            final MoveTransition moveTransition=board.currentPlayer().makeMove(move);
            if(moveTransition.getMoveStatus().isDone()){
                currentHighest=Math.max(currentHighest,min(moveTransition.getToBoard(),depth-1,highest,lowest));
    ...
    public static class MoveScoreRecord implements Comparable<MoveScoreRecord>{
        final Move move;
        final int score;
    ...
    enum Ordering{
        ...
        abstract List<MoveScoreRecord> order(List<MoveScoreRecord> moveScoreRecords);
    }
